...
public class UpdateLocalDb {
  ...
  // private static String KEY_ITEM_BRAND = "brand";
    private static String KEY_ITEM_BRAND = "item_brand";
    private static String KEY_ITEM_NAME = "item_name";
    private static String KEY_ITEM_DESCRIPTION = "item_description";
    ...
    private static String KEY_UNIQUE_PLACE_ID = "shop_id"; // Unique place id
    ...
    private static String KEY_IMAGE_DATA = "image_data"; // Image data
 ...
 //   public void Update(String user_unique_id, String barcode, String unique_place_id, Uri image_Path, String item_followed_at) {
  public void Update(ArrayList<NameValuePair> params) {
       ...
       //          params_reg.add(new BasicNameValuePair("item_followed_at", item_followed_at));
          ...
          //       Log.d("mycompany.myapp", "Smo pred jsonParser v ItemFollow!");	
              ...
              //    new JSONParser_IN().execute(registerURL, params_reg);
                 new JSONParser_IN().execute(params);
 ...
 //   	public final int THREAD_PRIORITY_BACKGROUND = 10;
     ...
     //   protected JSONObject doInBackground(Object... paramts) {
        ...
        //    List<NameValuePair> params = (List<NameValuePair>) paramts[1];
            ...
            Log.d("setEntity v UpdateLocalDb je: Paramts[0] je: ", paramts[0].toString());
...
*/
    ...
    */		
            ...
            //		}
                    ...
                    e.printStackTrace();
            //        Log.e("getOutputStream Error", "Error je: " + e.toString());
                    String ErrorUpdateToast = "Cannot sync. Server unreachable :-(";
                    //   Toast.makeText(MainActivity.context, ErrorFollowToast, Toast.LENGTH_LONG).show();
       				   Toast.makeText(context, ErrorUpdateToast, Toast.LENGTH_LONG).show();
                }
   ...
   //             conn.disconnect();
            ...
            try {
               // jObj = new JSONObject(json);
            	if (json_str.isEmpty()) {
            		Log.e("JSON Parser", "json_str je prazen v UpdateLocalDb!");
      ...
      //  protected void onPostExecute(JSONObject result) {
        protected void onPostExecute(JSONObject result) {
        	 Log.d("UpdateLocalDb", "Smo v UpdateLocalDb, v onPostExecute");
            //In here, call back to Activity or other listener that things are done
           //mCallback.onRequestCompleted(result);
        	if (result == null) {
				// do nothing
		//		String ErrorFollowToast = "Cannot follow item. Cannot connect to server :-(";
				String ErrorFollowToast = "A problem occurred during syncing.";
             //   Toast.makeText(MainActivity.context, ErrorFollowToast, Toast.LENGTH_LONG).show();
				   Toast.makeText(context, ErrorFollowToast, Toast.LENGTH_LONG).show();
			} else {
        	 ...
        	 try {
        		 json = result;
        		 Log.d("UpdateLocalDb", "json v UpdateLocalDb je: " +json.toString());
         //        if (json.getString(KEY_SUCCESS) != null) {
        		 Log.d("UpdateLocalDb", "json.getString(KEY_SUCCESS) v UpdateLocalDb je: " +json.getString(KEY_SUCCESS));
        		 Log.d("UpdateLocalDb", "json.getString(KEY_UPTODATE) v UpdateLocalDb je: " +json.getString(KEY_UPTODATE));
        	  //   if ((json.getString(KEY_SUCCESS) != null) && (json.getString(KEY_UPTODATE) == "false")) {
        		  if ((json.getString(KEY_SUCCESS) != null) && (json.getString(KEY_UPTODATE).equals("false"))) {
        		//   if ((json.getString(KEY_SUCCESS) != null) ) {
        	    //	 Log.d("UpdateLocalDb", "Smo notri!");
                 	Log.d("mycompany.myapp", "KEY_SUCCESS v UpdateLocalDb je uspeu!");
                    // registerErrorMsg.setText("");
                     String res = json.getString(KEY_SUCCESS); 
                     Log.d("mycompany.myapp", "json.getString v UpdateLocalDb je uspeu!");
                     Log.d("mycompany.myapp", "res v UpdateLocalDb je: " +res);
                     if(Integer.parseInt(res) == 1){
                         Log.d("mycompany.myapp", "Integer.parseInt(res) v UpdateLocalDb je uspeu!");
                         // user successfully registred
                         // Store user details in SQLite Database
             ///            DatabaseHandler db = new DatabaseHandler(getApplicationContext());
             //            SQLiteHandler db = new SQLiteHandler(getApplicationContext());
                  ...
                  //       userFunction.logoutUser(getApplicationContext());*/
                    	 int i = 0;
                    	 if (json.getJSONObject(String.valueOf(i)) == null ) {
                    		 Log.d("mycompany.myapp", "String.valueOf(i) v UpdateLocalDb je null!");
                    	 }
             //       	 while (json.getJSONObject(String.valueOf(i)) != null && !(json.getJSONObject(String.valueOf(i))).toString().isEmpty()) {
                       	 while (json.optJSONObject(String.valueOf(i)) != null) {
                       		 
                   	      Log.d("mycompany.myapp", "String.valueOf(i) v UpdateLocalDb ni null!");
                   	   Log.d("mycompany.myapp", "i v while zanki je: " +i);
                    		 //JSONObject json_item = json.getJSONObject("item");
                    		 JSONObject json_item = json.getJSONObject(String.valueOf(i));
                    		 fileUri = getOutputMediaFileUri(MEDIA_TYPE_IMAGE);
                    		 imagePath = ItemFollow.getPath(context, fileUri);
                    		  Log.d("mycompany.myapp", "imagePath i = " + i + " v UpdateLocalDb je: " +imagePath);
                    	//	 imagePath = "kabum" + String.valueOf(i);
                    	//	    Log.d("mycompany.myapp", "json_item.getString(KEY_BARCODE) v UpdateLocalDb je: " +json_item.getString(KEY_BARCODE));
                    		    db.addItem(json_item.getString(KEY_BARCODE),json_item.getString(KEY_ITEM_BRAND), json_item.getString(KEY_ITEM_NAME), json_item.getString(KEY_ITEM_DESCRIPTION), json_item.getDouble(KEY_ITEM_PRICE), json_item.getString(KEY_PRICE_CURRENCY), json_item.getInt(KEY_ITEM_DISCOUNT), json_item.getString(KEY_UNIQUE_ID), json_item.getString(KEY_UNIQUE_PLACE_ID), json_item.getString(KEY_ITEM_FOLLOWED_AT), imagePath,  0);
                    		    Qrimage = json_item.getString(KEY_IMAGE_DATA);
 ...
 /////////////////////                        db.deleteSingleItem(barCode, uniquePlaceId);
            //             db.deleteItems();   // TA JE SAMO ZA DEVELOPMENT!!! JE TREBA ZAKOMENTIRAT POTEM!
                    ...
                    //     db.addItem(json_item.getString(KEY_BARCODE),json_item.getString(KEY_ITEM_BRAND), json_item.getString(KEY_ITEM_NAME), json_item.getString(KEY_ITEM_DESCRIPTION), "NOW()", json_item.getDouble(KEY_ITEM_PRICE), json_item.getInt(KEY_ITEM_DISCOUNT), json_item.getString(KEY_UNIQUE_ID), json_item.getString(KEY_UNIQUE_PLACE_ID), imagePath,  0);
                     //    db.addItem(json_item.getString(KEY_BARCODE),json_item.getString(KEY_ITEM_BRAND), json_item.getString(KEY_ITEM_NAME), json_item.getString(KEY_ITEM_DESCRIPTION), json_item.getDouble(KEY_ITEM_PRICE), json_item.getInt(KEY_ITEM_DISCOUNT), json_item.getString(KEY_UNIQUE_ID), json_item.getString(KEY_UNIQUE_PLACE_ID), imagePath,  0);
                   //      db.close();
                    ...
                    //     String WelcomeToast = "Following item " .concat(json_item.getString("item_name").toString()).concat("!");
                       //  String WelcomeToast = "Following " .concat(json_item.getString("name").toString()).concat("!");
                         ...
                         Toast.makeText(context, "Items fetched to device", Toast.LENGTH_LONG).show();
                         Log.d("UpdateLocalDb", "Items fetched to device");
  ...
  //                       context.startActivity(UploadPicIntent);
                     ...
                     } else if (json.getString(KEY_SUCCESS) != null && json.getString(KEY_UPTODATE) != null) {
                   	   Log.d("mycompany.myapp", "KEY_SUCCESS je uspel v UpdateLocalDb, lokalna baza je up to date!");
                       Toast.makeText(context, "Local database up to date already", Toast.LENGTH_LONG).show();
                     } else{
                         // Error in registration
                         registerErrorMsg.setText("Sync error");
                //         Log.e("mycompany.myapp", "Already following item or item not in database");
                         String UpdateToast = "Sync error"; 
                         Toast.makeText(context, UpdateToast, Toast.LENGTH_LONG).show();
                     }
                 }
             } catch (JSONException e) {
             	Log.e("mycompany.myapp", "KEY_SUCCESS ni uspel v UpdateLocalDb!");
                 e.printStackTrace();
             }
        }
